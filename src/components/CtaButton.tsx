  ...
  if (locale === 'ru') {
    return (
      <a
        ...
        className={`${combinedClasses} relative pr-14`}
        >
          {/* Текст по центру — обычный, не absolute */}
          <span className="font-regular font-['Golos_Text']">Обсудить</span>

          {/* Иконка absolute прибита к правому краю */}
          <div className="absolute right-2 w-11 h-11 bg-white rounded-full flex items-center justify-center text-[#FF0033] flex-shrink-0 overflow-hidden z-0">
            <div className="absolute inset-0 z-0 pointer-events-none opacity-0 group-hover:opacity-30 transition-opacity duration-300">
              <div className="animate-speed-streak absolute w-[2px] h-[12px] bg-[#FF0033] rounded-full opacity-0" style={{ top: '0%', left: '50%', animationDelay: '0s' }}/>
              <div className="animate-speed-streak absolute w-[2px] h-[18px] bg-[#FF0033] rounded-full opacity-0" style={{ top: '20%', right: '20%', animationDelay: '0.2s' }}/>
              <div className="animate-speed-streak absolute w-[2px] h-[8px] bg-[#FF0033] rounded-full opacity-0" style={{ top: '-10%', right: '45%', animationDelay: '0.35s' }}/>
            </div>
