...
interface CtaButtonProps {
  locale: string;
  className?: string; // Позволяет прокидывать доп. стили снаружи
}
...
export default function CtaButton({ locale, className = "" }: CtaButtonProps) {
  ...
  // Базовые стили, которые есть у кнопки ВСЕГДА (цвет, скругление, флекс)
  ...
  // Объединяем базы и то, что ты передашь снаружи (например, размеры)
  const combinedClasses = `${baseStyles} ${className}`;

  // --- ЛОГИКА ДЛЯ РУССКИХ (Телеграм) ---
  if (locale === 'ru') {
    return (
      <a
      ...
      >
        ...
        <div className="w-8 h-8 md:w-11 md:h-11 bg-white rounded-full flex items-center justify-center text-[#FF0033] flex-shrink-0 transition-transform">
          <svg viewBox="0 0 20 17" fill="currentColor" className="w-4 h-3.5 md:w-5 md:h-4.5 relative -left-[1px]">
  ...
  // --- ЛОГИКА ДЛЯ ОСТАЛЬНЫХ (Копирование Email) ---
